...
BST_Node *newBST_Node(double freq, int bar, double index)
{
     ...
     ****/
	 ...
	 BST_Node *new_BST = NULL;
	 
	 new_BST = (BST_Node *)calloc(1, sizeof(BST_Node));
	 
	 new_BST->key = (10.0*bar)+index;
	 new_BST->freq = freq;
	 new_BST->bar = bar;
	 new_BST->index = index;
	 new_BST->left = NULL;
...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
     ...
     ****/
	 ...
	 double find_key = (10.0*bar)+index;
	 
	 if (root == NULL) return NULL;
	 
	 if (fabs(root->key - find_key) < 0.0001){
		 return root;
	 } 
	 else if (root->key > find_key){
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
     ...
     ****/
	 ...
	 BST_Node *temp = NULL;	 
	 double given_key = (10.0*bar)+index;
	 
	 if (root == NULL){
		 return NULL;
	 }
	 
	 if (fabs(root->key - given_key) < 0.0001){
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
     ...
     ****/
	 ...
	 int i;
	 double freq_to_find;
	 double change_freq_to;
	 
	 for(i=0; i<100; i++){
		if (strcmp(note_names[i], note_src) == 0){
			freq_to_find = note_freq[i];
		}
		 
		if (strcmp(note_names[i], note_dst) == 0){
			change_freq_to = note_freq[i];
		}
	 }
	 ...
	 if (root != NULL){
		...
		if (fabs(root->freq - freq_to_find) < 0.0001){
			root->freq = change_freq_to;
		}
...
double find_freq(BST_Node *root, int semitones)
{
	/* Finds the frequency of the given root and changes it to
	the frequency of the note at the root_freq[index+semitones]
	*/
	int i;
	double new_freq = 0;
	...
	for (i=0; i<100; i++)
	{
		...
		{
			if(0 <= (i+semitones) && (i+semitones) <= 99)
			{
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     *       then yo= DO NOT insert the new note.
     * 
     ...
     ****/
	 ...
	 double new_index = 0.0;	
	 double new_freq = 0.0;
	 BST_Node *new_node = NULL;
	 	 
	 if (root == NULL) return NULL;
	
	 if ((root->index + time_shift) >= 0 && (root->index + time_shift) < 1 && (new_freq >= 0 &&  new_freq <= 99)){
		 new_index = root->index + time_shift;
		 new_freq = find_freq(root, semitones);
		 new_node = newBST_Node(new_freq, root->bar, new_index);
		 
		 if (BST_search(new_node, new_node->bar, new_index) != NULL){
			new_node = newBST_Node(new_freq, root->bar, new_index+0.000001);
